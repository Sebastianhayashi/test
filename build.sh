...
# Save the install log to a file for debugging
echo "$INSTALL_LOG" > ~/gz_common_install_log.txt
...
# Save the environment variables to a file for debugging
echo "IGNITION_COMMON3_INCLUDE_DIR: $IGNITION_COMMON3_INCLUDE_DIR" > ~/env_vars.txt
echo "IGNITION_COMMON3_LIBRARY: $IGNITION_COMMON3_LIBRARY" >> ~/env_vars.txt
echo "IGNITION_COMMON3_DIR: $IGNITION_COMMON3_DIR" >> ~/env_vars.txt
...
if [ $? -eq 0 ]; then
    ...
    # Save the CMake output to a file for debugging
    cmake -DIGNITION_COMMON3_DIR=$IGNITION_COMMON3_DIR \
          -DIGNITION_COMMON3_INCLUDE_DIR=$IGNITION_COMMON3_INCLUDE_DIR \
          -DIGNITION_COMMON3_LIBRARY=$IGNITION_COMMON3_LIBRARY \
          -DCMAKE_LIBRARY_PATH=/usr/local/lib64 \
